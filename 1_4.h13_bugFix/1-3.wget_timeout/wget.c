...
{
	bb_error_msg("set timeout_seconds end %d", G.timeout_seconds);
	bb_error_msg("alarm_handler timeout");
	bb_error_msg("G.connecting %d\n", G.connecting);
...
static char fgets_and_trim(FILE *fp)
{
	...
	char *buf_ptr;
        ...
        bb_error_msg("send request func: %d  line: %d", __func__, __LINE__); 
...
{
    bb_error_msg("retrieve_file_data"); 
...
# endif
	...
	while (1) {
        ...
        bb_error_msg("send request func: %d  line: %d", __func__, __LINE__); 
#if ENABLE_FEATURE_WGET_STATUSBAR || ENABLE_FEATURE_WGET_TIMEOUT
            bb_error_msg("second_cnt: %d", second_cnt); 
...
static void download_one_url(const char *url)
{
 ...
 establish_session:
	...
	if (use_proxy || !target.is_ftp) {
        ...
        bb_error_msg("send request func: %d  line: %d", __func__, __LINE__); 
        bb_error_msg("send response func: %d  line: %d", __func__, __LINE__); 
		/*
 ...
 read_response:
		...
		status = atoi(str);
        bb_error_msg("send request func: %d  line: %d, %d", __func__, __LINE__, status); 
		switch (status) {
		case 0:
		case 100:
